<x-layouts.admin>
    ...
    <x-slot:subtitle>Buat produk baru ke dalam katalog</x-slot:subtitle>
                    ...
                    <div class="space-y-5 relative z-10">
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name') }}" required maxlength="255"
                                placeholder="Contoh: Caramel Macchiato" class="form-input transition-colors focus:ring-2 focus:ring-blue-100 focus:border-blue-500 text-sm">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" placeholder="Ceritakan detail rasa, komposisi, atau keunggulan menu ini kepada pelanggan..."
                                class="form-input transition-colors focus:ring-2 focus:ring-blue-100 focus:border-blue-500 text-sm leading-relaxed">{{ old('description') }}</textarea>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 relative z-10">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="price" name="price" value="{{ old('price') }}" required min="0" max="999999999" step="100" placeholder="25000" 
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="stock" name="stock" value="{{ old('stock', 100) }}" required min="0" max="999999" placeholder="100" 
            ...
            <div class="space-y-6">
                ...
                <div class="admin-card p-6 border-t-4 border-t-purple-500 shadow-sm relative overflow-hidden">
                    <h2 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                        ...
                        <div id="image-preview-container" class="mb-4 bg-purple-50/50 rounded-xl p-3 border border-purple-100 relative overflow-hidden hidden">
                            <div class="flex items-center justify-between mb-2">
                                <div class="z-10">
                                    <p class="text-sm font-bold text-purple-900 leading-tight">Preview Gambar Baru</p>
                                    <p id="file-count-display" class="text-[10px] text-purple-600 mt-0.5 font-medium"></p>
                                </div>
                                <button type="button" onclick="cancelImageSelection(event)" class="w-8 h-8 flex items-center justify-center rounded-full bg-red-50 text-red-500 hover:bg-red-100 transition-colors z-20">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                </button>
                            </div>
                            
                            <!-- Carousel -->
                            <div class="relative w-full overflow-hidden rounded-lg bg-white border border-purple-200">
                                <div id="carousel-track" class="flex transition-transform duration-300 ease-in-out">
                                    <!-- Images will be injected here -->
                                </div>
                                <!-- Prev Button -->
                                <button type="button" onclick="moveCarousel(-1)" class="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center rounded-full bg-black/50 text-white hover:bg-black/70 z-20 hidden shadow-md focus:outline-none focus:ring-2 focus:ring-white" id="carousel-prev">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                                </button>
                                <!-- Next Button -->
                                <button type="button" onclick="moveCarousel(1)" class="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center rounded-full bg-black/50 text-white hover:bg-black/70 z-20 hidden shadow-md focus:outline-none focus:ring-2 focus:ring-white" id="carousel-next">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                                </button>
                            </div>
                            <!-- Thumbnails/Dots -->
                            <div id="carousel-indicators" class="flex justify-center flex-wrap gap-1.5 mt-3 px-2">
                            </div>
                        </div>

                        <div id="upload-dropzone" class="mt-2 flex justify-center rounded-xl border-2 border-dashed border-gray-300 px-6 py-8 bg-gray-50/50 hover:bg-purple-50 hover:border-purple-300 transition-all duration-200 relative group cursor-pointer" onclick="document.getElementById('images').click()">
                            <div class="text-center">
                                <div class="bg-white w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-3 shadow-sm border border-gray-100 group-hover:scale-110 transition-transform">
                                    <svg class="h-6 w-6 text-purple-500" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                                        <path fill-rule="evenodd" d="M1.5 6a2.25 2.25 0 012.25-2.25h16.5A2.25 2.25 0 0122.5 6v12a2.25 2.25 0 01-2.25 2.25H3.75A2.25 2.25 0 011.5 18V6zM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0021 18v-1.94l-2.69-2.689a1.5 1.5 0 00-2.12 0l-.88.879.97.97a.75.75 0 11-1.06 1.06l-5.16-5.159a1.5 1.5 0 00-2.12 0L3 16.061zm10.125-7.81a1.125 1.125 0 112.25 0 1.125 1.125 0 01-2.25 0z" clip-rule="evenodd" />
                                    </svg>
                                </div>
                                <div class="mt-2 text-sm leading-6 text-gray-600">
                                    <span class="font-bold text-purple-600">Tekan Untuk Upload File <br>(Bisa Pilih Banyak Foto)</span>
                                ...
                                <p class="text-[11px] leading-5 text-gray-400 mt-2">Hanya PNG, JPG, WEBP (Maks: 5 File, @2MB)</p>
                            </div>
                        </div>
                    ...
                    <div class="space-y-4">
                        <label class="flex items-start gap-3 p-3.5 rounded-xl border border-gray-100 hover:bg-green-50/30 hover:border-green-200 cursor-pointer transition-all group">
                            ...
                            <div class="flex items-center h-5 mt-0.5">
                                <input type="checkbox" name="is_available" value="1" checked class="w-4 h-4 rounded border-gray-300 text-green-500 focus:ring-green-500 transition-colors">
                            </div>
                        ...
                        <label class="flex items-start gap-3 p-3.5 rounded-xl border border-amber-100 bg-amber-50/40 hover:bg-amber-50 hover:border-amber-200 cursor-pointer transition-all group">
                            ...
                            <div class="flex items-center h-5 mt-0.5">
                                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="w-4 h-4 rounded border-amber-300 text-amber-500 focus:ring-amber-500 transition-colors">
                            </div>
                            <div class="flex flex-col">
                        ...
                        <label class="flex items-start gap-3 p-3.5 rounded-xl border border-blue-100 bg-blue-50/40 hover:bg-blue-50 hover:border-blue-200 cursor-pointer transition-all group">
                            ...
                            <div class="flex items-center h-5 mt-0.5">
                                <input type="checkbox" name="is_newcomer" value="1" {{ old('is_newcomer') ? 'checked' : '' }} class="w-4 h-4 rounded border-blue-300 text-blue-500 focus:ring-blue-500 transition-colors">
                            </div>
                            <div class="flex flex-col">
                ...
                <div class="admin-card p-6 border-t-4 border-t-gray-800 shadow-sm relative overflow-hidden">
                    <h2 class="text-lg font-bold text-gray-800 mb-2">Simpan Perubahan</h2>
                    <p class="text-xs text-gray-500 mb-5 leading-relaxed">Pastikan semua kolom vital telah diisi dengan benar sebelum mem-publish ke database.</p>
                    <div class="flex flex-col gap-3">
                        <button type="submit" class="btn-primary w-full justify-center py-2.5 shadow-md hover:shadow-lg transition-all text-sm font-bold">
                            <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                            </svg>
                            Tanam ke Katalog
                        </button>
                        <a href="{{ route('admin.products.index') }}" class="btn-secondary w-full justify-center py-2.5 text-sm font-bold bg-white text-gray-700 hover:bg-gray-100 border border-gray-200">Batal</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
            ...
            if (input.files && input.files.length > 0) {
                ...
                Array.from(input.files).forEach((file, index) => {
                    ...
                    reader.onload = function(e) {
                        ...
                        dot.type = 'button';
                        dot.className = `h-2 rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-purple-300 ${index === 0 ? 'bg-purple-600 w-4' : 'bg-purple-300 w-2'}`;
        ...
        function updateCarousel() {
            ...
            for(let i=0; i<dots.length; i++) {
                if(i === currentSlide) {
                    dots[i].classList.replace('bg-purple-300', 'bg-purple-600');
                    dots[i].classList.replace('w-2', 'w-4');
                } else {
                    dots[i].classList.replace('bg-purple-600', 'bg-purple-300');
        ...
        function cancelImageSelection(event) {
            ...
            // clear track to explicitly release somewhat heavy DOM elements holding base64 string
