...
{{-- ╔═══════════════════════════════════════════╗
     ...
     ╚═══════════════════════════════════════════╝ --}}
<div class="relative bg-gradient-to-br from-amber-700 via-amber-600 to-amber-500 text-white overflow-hidden
    ...
    <div class="absolute -left-12 bottom-0 w-48 h-48 bg-amber-900/20 rounded-full blur-2xl pointer-events-none"></div>
    <div class="absolute right-0 bottom-0 w-40 h-40 bg-amber-300/10 rounded-full blur-xl pointer-events-none"></div>
    ...
    <div class="relative z-10 px-5 pt-10 pb-6 md:px-10 md:pt-12 md:pb-10 lg:px-14 lg:pt-14 lg:pb-12 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        ...
        {{-- Left: Greeting + Info --}}
        <div class="max-w-xl">
            <p class="text-amber-200 text-xs md:text-sm font-semibold uppercase tracking-widest mb-1 md:mb-2">Hai, Penikmat Kopi ☕</p>
            <h1 class="text-2xl md:text-4xl lg:text-5xl font-extrabold tracking-tight leading-tight mb-3">{{ $shopName }}</h1>
            <p class="text-amber-100 text-sm md:text-base leading-relaxed mb-5 opacity-90 max-w-sm">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('menu.index') }}"
                   ...
                   class="inline-flex items-center gap-2 bg-white text-amber-700 px-5 py-2.5 rounded-2xl text-sm font-bold shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:scale-95 transition-all">
                    Pesan Sekarang
        ...
        <div class="hidden md:block flex-shrink-0 w-72 lg:w-80 bg-white/15 backdrop-blur-md border border-white/25 rounded-2xl p-5 shadow-xl">
            <p class="text-amber-200 text-[10px] font-bold uppercase tracking-widest mb-2">Promo Hari Ini</p>
            ...
            <div class="flex items-center gap-2 text-amber-100 text-xs">
                <svg class="w-4 h-4 flex-shrink-0 text-amber-300" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"/></svg>
    ...
    <div class="md:hidden relative z-10 mx-5 mb-5 bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-4">
        <p class="text-amber-100 text-[10px] font-bold uppercase tracking-wider mb-1">Promo Spesial</p>
...
<section class="mt-8 md:mt-10 mb-4 md:mb-6 px-5 md:px-6 lg:px-10">
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-base md:text-lg font-extrabold text-slate-800">Kategori Menu</h3>
        <a href="{{ route('menu.index') }}" aria-label="Lihat Semua Kategori" class="text-xs md:text-sm font-semibold text-amber-600 hover:text-amber-700 transition-colors">Lihat Semua →</a>
    </div>
    ...
    <div class="flex overflow-x-auto no-scrollbar md:overflow-visible md:flex-wrap gap-2.5 md:gap-3 pb-2 md:pb-0 snap-x md:snap-none">
        <a href="{{ route('menu.index') }}"
           ...
           class="flex flex-col items-center gap-2 flex-shrink-0 md:flex-shrink snap-start active:scale-95 md:hover:scale-105 transition-transform">
            <div class="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-amber-50 border border-amber-100 flex items-center justify-center text-amber-500 shadow-sm">
            ...
            <span class="text-[10px] md:text-xs font-semibold text-slate-600">Semua</span>
        </a>
        ...
        <a href="{{ route('menu.index', ['category' => $category->slug]) }}"
           ...
           class="flex flex-col items-center gap-2 flex-shrink-0 md:flex-shrink snap-start active:scale-95 md:hover:scale-105 transition-transform">
            ...
            <span class="text-[10px] md:text-xs font-semibold text-slate-600 truncate max-w-[56px] md:max-w-[64px]">{{ $category->name }}</span>
        </a>
        @endforeach
    </div>
</section>
...
<section class="mt-2 mb-8 bg-gradient-to-r from-slate-50 to-amber-50/60 py-6 md:py-8 border-y border-slate-100">
    <div class="flex items-center justify-between px-5 md:px-6 lg:px-10 mb-5">
        <h3 class="text-base md:text-lg font-extrabold text-slate-800 flex items-center gap-2">
        ...
        <a href="{{ route('menu.show', $product->slug) }}"
           ...
           class="flex-shrink-0 w-[150px] md:w-full bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden snap-start group relative hover:-translate-y-1 transition-all duration-300">
            ...
            <div class="absolute top-2.5 left-2.5 z-10">
                <span class="px-2 py-0.5 bg-gradient-to-r from-red-500 to-rose-500 text-white text-[9px] font-bold uppercase rounded-md shadow">NEW</span>
            </div>
            ...
            <div class="p-3 md:p-4">
                ...
                <h4 class="text-xs md:text-sm font-bold text-slate-800 leading-snug mb-2 line-clamp-2">{{ $product->name }}</h4>
                <div class="flex items-center justify-between">
                    <p class="text-sm md:text-base font-extrabold text-amber-600">{{ $product->formatted_price }}</p>
                    <span class="w-6 h-6 rounded-full bg-amber-50 text-amber-500 flex items-center justify-center group-hover:bg-amber-500 group-hover:text-white transition-colors">
               ...
               class="flex gap-4 p-3.5 md:p-4 bg-white rounded-2xl shadow-sm border border-slate-100 items-center hover:shadow-md hover:-translate-y-0.5 active:bg-slate-50 transition-all duration-200">
                ...
                <div class="flex-1 min-w-0 py-0.5">
                    <div class="flex justify-between items-start mb-1">
                        <h4 class="text-sm md:text-base font-bold text-slate-800 truncate pr-2">{{ $product->name }}</h4>
                        @if($product->rating_count > 0)
                            <div class="flex items-center gap-0.5 text-[10px] font-bold text-amber-500 bg-amber-50 px-1.5 py-0.5 rounded-lg flex-shrink-0" aria-label="Rating {{ number_format($product->rating_avg, 1) }} dari 5">
                    ...
                    <div class="flex items-center justify-between">
                        <p class="text-sm md:text-base font-extrabold text-amber-600">{{ $product->formatted_price }}</p>
                        <div class="w-7 h-7 bg-slate-800 text-white rounded-full flex items-center justify-center shadow-md hover:bg-amber-600 transition-colors">
