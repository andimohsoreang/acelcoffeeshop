<x-layouts.app title="Pesanan Berhasil">
    ...
    <div class="px-4 py-6 md:px-10 md:py-12 max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-2xl md:rounded-3xl border border-slate-100 shadow-sm overflow-hidden mb-5 md:mb-6">
            <div class="bg-brand-primary px-4 py-3 md:px-6 md:py-4 text-center">
                <p class="text-brand-secondary/80 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-0.5 md:mb-1">Nomor Antrean Anda</p>
            ...
            <div class="p-4 md:p-6">
                <div class="flex flex-wrap items-center justify-between gap-3 md:gap-4 mb-4 pb-4 border-b border-dashed border-slate-200">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-base md:text-lg font-black text-brand-primary">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-y-3 gap-x-4 md:gap-y-4 md:gap-x-6">
                    ...
                    <div>
                        ...
                        @else
                            <span class="inline-flex items-center gap-1 bg-brand-secondary text-brand-primary text-[9px] md:text-[10px] font-bold px-1.5 md:px-2 py-0.5 md:py-1 rounded md:rounded-md uppercase">
                    ...
                    @if($qris && $qris->image)
                        <div class="bg-white rounded-xl md:rounded-2xl p-3 md:p-4 border border-blue-100 mb-4 md:mb-5 text-center">
                            ...
                            <div x-data="{ showQrisModal: false }" class="inline-block relative">
                                <img @click="showQrisModal = true" src="{{ asset('storage/' . $qris->image) }}" alt="QRIS" loading="lazy" width="250" height="250" class="w-48 h-48 md:w-64 md:h-64 object-contain mx-auto rounded-lg md:rounded-xl border border-slate-100 p-1.5 md:p-2 shadow-sm cursor-pointer hover:border-brand-primary/30 transition-colors">
                                
                                <div class="mt-3 flex items-center justify-center gap-2">
                                    <button @click="showQrisModal = true" type="button" class="inline-flex items-center gap-1.5 text-[10px] md:text-xs font-bold text-brand-primary hover:text-brand-dark bg-brand-secondary hover:bg-brand-secondary/80 px-3 py-1.5 rounded-lg transition-colors">
        ...
        @elseif($order->payment->method === 'cash')
            <div class="bg-brand-secondary rounded-2xl md:rounded-3xl border border-brand-primary/10 p-4 md:p-5 mb-5 md:mb-6 text-center">
                <svg class="w-6 h-6 md:w-8 md:h-8 text-brand-primary mx-auto mb-1.5 md:mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                <p class="text-xs md:text-sm text-brand-dark font-bold mb-0.5 md:mb-1">Pembayaran Tunai Kasir</p>
                <p class="text-[10px] md:text-xs text-brand-muted">Sebutkan <strong>Nomor Antrean</strong> Anda saat di kasir.</p>
            </div>
        @endif

        {{-- Action Buttons --}}
        <div class="flex flex-col sm:flex-row gap-2 md:gap-3">
            <a href="{{ route('order.show', $order->order_code) }}" class="flex-1 text-center bg-white border-2 border-brand-primary text-brand-primary hover:bg-brand-secondary font-bold py-3 md:py-3.5 px-4 rounded-xl md:rounded-2xl text-xs md:text-sm transition-colors">
